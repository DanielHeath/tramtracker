...
// jammed together :/
func NewGapWatcher(q Query, gapsize time.Duration) (Watcher, chan Gap) {
	out := make(chan Gap)
	return Watcher{
		Query:             q,
...
func gapIdentifier(gapsize time.Duration, out chan Gap) func(TrackerResponse) {
	return func(tt TrackerResponse) {
...
type Watcher struct {
	...
	OnEventRecognized func(TrackerResponse)
	OnErr             func(error)
	Ticker            <-chan time.Time
	alreadySeen       map[interface{}]bool
}

type eventRecognizer func(TrackerResponse) interface{}
...
func getLastTramId(tt TrackerResponse) interface{} {
